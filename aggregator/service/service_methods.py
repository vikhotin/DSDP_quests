...
def GetUserInfo(request, user_login):
    try:
...
def GetUserQuestInfo(quest_id, user_login):
    try:
        res = requests.get(user_service_address + '/user/{}/'.format(user_login))
    except requests.exceptions.ConnectionError:
        return JsonResponse({'error': 'Service currently unavailable'}, status=503)
...
def PostUserQuest(quest_id, request, user_login):
    try:
        res = requests.get(user_service_address + '/user/{}/'.format(user_login))
    except requests.exceptions.ConnectionError:
        return JsonResponse({'error': 'Service currently unavailable'}, status=503)
...
def GetPlaceInfo(fact_id, place_id, quest_id, user_login):
    try:
        res = requests.get(user_service_address + '/user/{}/'.format(user_login))
    except requests.exceptions.ConnectionError:
        return JsonResponse({'error': 'Service currently unavailable'}, status=503)
